...
//                .map(test.src.src.Song::getArtist)
//                .filter(artist->!artist.equals("The Beatles"))
//                .collect(Collectors.toList());
//        List<test.src.src.Song> rockmusic = songs.stream()
//                .filter(song -> song.getArtist().equals("The Beatles"))//filter for The Beatles
//                .collect(Collectors.toList());

        List<String> rockmusic = songs.stream()
                .filter(song -> song.getGenre().equals("Rock"))//for latin music
                .map(Song::getArtist).toList();
//        List<test.src.src.Song> rockmusic=songs.stream()
//                .filter(song->song.getArtist().startsWith("H"))//filter for bands starting with H
//                .collect(Collectors.toList());
//        List<test.src.src.Song> rockmusic=songs.stream()
//                .filter(song->song.getYear()>1995)//filter for music newer than 1995
//                .collect(Collectors.toList());
//        List<String> genres=songs.stream()
//                .map(song->song.getGenre())//filter for music newer than 1995
//                .distinct()
//                .collect(Collectors.toList());
...
class Songs {
    public List<Song> getSongs() {
        return List.of(
                ...
                new Song("Immigrant test.src.src.Song", "Karen O", "Industrial Rock", 2011, 12),
